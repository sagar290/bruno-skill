...
*   When the agent develops a new API route in the backend code, it runs `sync_bruno.py`.
...
*   Our `sync_bruno.py` script has a smart merging algorithm that parses and preserves your custom edits (headers, body parameters, tests) and only updates the base routing URL/method when code changes.
